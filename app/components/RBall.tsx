...
type PositionTime = [Position, Position]; // [currentPosition, newPosition]
type Action = 
...
const getRandomPos = (currentPos: Position, windowSize: Size): Position => {
  while (true) {
    ...
    const randPos: Position = windowSize ? [ 
      Math.floor(getRandom(windowSize.width ? [windowSize.width * 0.15, windowSize.width * 0.7] : undefined)), // width
      Math.floor(getRandom(windowSize.height ? [windowSize.height * -0.1, windowSize.height * 0.6] : undefined)) // height
    ] : initialPos;
    ...
    case "animationEnded":
      return { 
        ...
        style: {
          ...state.style,
          pointerEvents: "all",
          left: state.poss[0][0] + 'px',
          bottom: state.poss[0][1] + 'px'
        }
      };
